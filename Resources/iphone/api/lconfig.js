...
function AddSelectedProduct(id) {
    alert("Entrou na funcao 1" + contX);
    alert("Z" + contZ);
    alert("W" + contW);
    var contY = contX + 1;
    alert("Entrou na funcao 2" + contX);
    if (contX == contY || 0 == contX && 0 == contW) {
        contX++;
        Ti.App.Properties.setString("X", contX);
        alert("Entrou na funcao 3" + contX);
        vet_aux = Ti.App.Properties.getList("valores");
        vet_aux = vet_aux.filter(function(este, i) {
            return vet_aux.indexOf(este) == i;
        });
        var val_melo = parseInt(Ti.App.Properties.getString("globals_value"));
        Ti.App.Properties.setString("globals_value", val_melo - 1);
        var products = [];
        if (Ti.App.Properties.getList(SELECTED_PRODUCTS)) var products = Ti.App.Properties.getList(SELECTED_PRODUCTS);
        var result = "false";
        if ("false" == checkSelectedProduct(id) && -1 == vet_aux.indexOf(id)) {
            alert("SIM");
            contZ++;
            Ti.App.Properties.setString("Z", contZ);
            products.push(id);
            result = "true";
            vet_aux.push(id);
            var vet_aux = vet_aux.filter(function(este, i) {
                return vet_aux.indexOf(este) == i;
            });
            Ti.App.Properties.setList("valores", vet_aux);
            Ti.App.Properties.setList(SELECTED_PRODUCTS, vet_aux);
        } else {
            alert("NAO");
            contW++;
            Ti.App.Properties.setString("W", contW);
            var merda = [];
            var index_prd = products.indexOf(id);
            products.splice(index_prd, 1);
            for (var i = 0; i < vet_aux.lenth; i++) vet_aux[i] != id && (merda[i] = vet_aux[i]);
            vet_aux = merda;
            result = "false";
            Ti.App.Properties.setList("valores", vet_aux);
            Ti.App.Properties.setList(SELECTED_PRODUCTS, vet_aux);
...
function getImagesFolder() {
    var rootdir;
    rootdir = "android" == Ti.Platform.osname ? Ti.Filesystem.externalStorageDirectory : Ti.Filesystem.applicationDataDirectory;
    return rootdir;
}
...
Ti.App.Properties.setList("valores", []);

var vet_aux = [];

Ti.App.Properties.setString("X", 0);

Ti.App.Properties.setString("Z", 0);

Ti.App.Properties.setString("W", 0);

var contX = parseInt(Ti.App.Properties.getString("X"));

var contZ = parseInt(Ti.App.Properties.getString("Z"));

var contW = parseInt(Ti.App.Properties.getString("W"));