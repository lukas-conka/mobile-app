...
function consultaUltimoCarrinhoPedido() {
    ...
    var result = 0;
    carrinhoPedido.isValidRow() && (result = carrinhoPedido.fieldByName("crp_id"));
    db.close();
    return result;
}
...
function consultaCarrinhoPedidoByPedido(pedido) {
    db = dbLoad();
    var query = "SELECT tb_carrinho_pedido.crp_quantidade, tb_carrinho_pedido.desconto_unit, tb_carrinho_pedido.crp_preco_unitario, tb_carrinho_pedido.crp_ipi, tb_carrinho_pedido.crp_data, tb_carrinho_pedido.desconto, tb_carrinho_pedido.desconto_parcela, tb_carrinho_pedido.desconto_especial, tb_carrinho_pedido.crp_forma_pagamento, tb_carrinho_pedido.fk_tamanhos, tb_carrinho_pedido.fk_produtos, tb_carrinho_pedido.fk_cores, tb_pedido.ped_entrega, tb_pedido.ped_entrega_prazo, tb_pedido.ped_numero, tb_pedido.ped_observacao, tb_pedido.fk_marcas, tb_pedido.ped_data_pag, tb_cliente.cl_cnpj, tb_cliente.cl_razao, tb_cliente.cl_cep_unid, tb_cliente.cl_end_unid, tb_cliente.cl_n_unid, tb_cliente.cl_comp_unid, tb_cliente.cl_bairro_unid, tb_cliente.cl_cidade_unid, tb_cliente.cl_uf_unid, tb_cores.cor_nome, tb_tamanhos.tmh_nome, tb_produtos.prd_nome, tb_produtos.prd_referencia, tb_produtos.prd_ipi, tb_produtos.prd_nome_colecao, tb_produtos.fk_marca,tb_produtos.prd_id, tb_info_produtos.ifp_peso, tb_info_produtos.ifp_cub_a, tb_info_produtos.ifp_cub_l, tb_info_produtos.ifp_cub_p FROM tb_carrinho_pedido INNER JOIN tb_pedido ON tb_pedido.ped_id = " + pedido + " INNER JOIN tb_cliente ON tb_cliente.cl_id = tb_pedido.fk_cli INNER JOIN tb_cores ON tb_cores.cor_id = tb_carrinho_pedido.fk_cores INNER JOIN tb_tamanhos ON tb_tamanhos.tmh_id = tb_carrinho_pedido.fk_tamanhos INNER JOIN tb_produtos ON tb_produtos.prd_id = tb_carrinho_pedido.fk_produtos INNER JOIN tb_info_produtos ON tb_info_produtos.fk_produtos = tb_carrinho_pedido.fk_produtos AND tb_info_produtos.fk_cores = tb_carrinho_pedido.fk_cores AND tb_info_produtos.fk_tamanhos = tb_carrinho_pedido.fk_tamanhos WHERE tb_carrinho_pedido.crp_fk_pedido = " + pedido + " ORDER BY tb_carrinho_pedido.fk_produtos;";
