...
function consultaPedidos() {
    db = dbLoad();
    var pedido = db.execute("SELECT tb_pedido.*,tb_cliente.cl_razao,tb_cliente.cl_cnpj, tb_pedido.ped_data FROM " + getTablePedido() + " INNER JOIN tb_cliente ON tb_cliente.cl_id = tb_pedido.fk_cli  ORDER BY ped_id DESC");
    db.close();
    return pedido;
}
...
function consultaPedidosPorData(inicio, fim) {
    db = dbLoad();
    var query = "SELECT tb_pedido.*,tb_cliente.cl_razao,tb_cliente.cl_cnpj, tb_pedido.ped_data FROM " + getTablePedido() + " INNER JOIN tb_cliente ON tb_cliente.cl_id = tb_pedido.fk_cli WHERE ped_data BETWEEN '" + inicio + "' AND '" + fim + "'  ORDER BY ped_id DESC";
...
function consultaUltimoPedido() {
    ...
    var result = 0;
    pedido.isValidRow() && (result = pedido.fieldByName("ped_id"));
    db.close();
    return result;
}