...
function insertClientes(cl_id, cl_nome_comprador, cl_cargo,
...
function getClienteCredito(cliente) {
	db = dbLoad();
	var clientes = db.execute('SELECT cl_razao,cl_credito_total,cl_credito_utilizado,cl_valor_minimo,cl_aviso,cl_observacao FROM ' + getTableClientes() +
	' WHERE cl_id = ' + cliente);
	if (Ti.Platform.osname == "android") {
		db.close();
	}
...
function processClientes(jsonTxt) {
	...
	for (var i = 0; i < jsonObject.length; i++) {
		...
		insertClientes(cl_id, cl_nome_comprador, cl_cargo,
		cl_razao, cl_fantasia, cl_unidade, cl_cnpj, cl_ie,
		cl_fone, cl_ddd, cl_cep_fa, cl_end_fa,
		cl_n_fa, cl_comp_fa, cl_bairro_fa, cl_cidade_fa, cl_uf_fa,
		cl_fone_fa, cl_fax_fa, email_cad, cl_entrega, cl_data,
