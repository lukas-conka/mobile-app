...
//Rare exceptions to indicate a bug in the _e_catalogos code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_e_catalogos = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_e_catalogos]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.e_catalogos.module"
